...
def random_positive_value():
    """
    Used to generate only positive values for
    tests and not cause any errors because of
    negative values.
    """
...
def payment(db):
    ...
    payment = baker.make(Payment, due_date=d, value=random_positive_value)
    return payment
...
def payment_supplier_01(supplier_01):
    ...
    payment_01 = baker.make(Payment, supplier=supplier_01, due_date=due_date, value=random_positive_value)
    return payment_01
...
def available_payments_user_01(db, supplier_01):
    ...
    payments_list = baker.make(Payment, due_date=due_date, _quantity=3,
                               supplier=supplier_01, value=random_positive_value)
...
def available_payments_user_02(db, supplier_02):
    ...
    due_date = date.today() + timedelta(days=5)
    payments_list = baker.make(Payment, due_date=due_date, _quantity=3,
                               supplier=supplier_02, value=random_positive_value)
    return payments_list
...
def unavailable_payments_user_01_due_date(db, supplier_01):
    ...
    """
    due_date = date.today()
    payment = baker.make(Payment, due_date=due_date, _quantity=2,
                         supplier=supplier_01, value=random_positive_value)
    return payment
...
def payment_user_01_anticipation_created(db, supplier_01):
    ...
    payment_ant = baker.make(Payment, due_date=due_date, supplier=supplier_01, value=random_positive_value)
...
def payment_user_02_anticipation_created(db, supplier_02):
    ...
    """
    d = date.today() + timedelta(days=5)
    payment = baker.make(Payment, supplier=supplier_02, due_date=d, value=random_positive_value)
...
def payment_user_02_anticipation_related_status_d(db, supplier_02):
    ...
    due_date = date.today() + timedelta(days=5)
    new_due_date = str(date.today() + timedelta(days=1))
    payment_ant = baker.make(Payment, due_date=due_date, supplier=supplier_02, value=random_positive_value)
    baker.make(Anticipation, payment=payment_ant, new_due_date=new_due_date, status='D')
    return payment_ant
...
def unavailable_payment(db):
    ...
    """
    today = date.today()
    unav_paym = baker.make(Payment, due_date=today, value=random_positive_value)
    return unav_paym
